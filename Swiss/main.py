...
tf.flags.DEFINE_integer("train_size", 1000, "Train over these many tweets (default: 1000)")

# Misc Parameters
tf.flags.DEFINE_boolean("allow_soft_placement", True, "Allow device soft device placement")
tf.flags.DEFINE_boolean("log_device_placement", False, "Log placement of ops on devices")
tf.flags.DEFINE_integer("num_cores", 7, "the cores of the cpu")
...
with tf.Graph().as_default():
	...
	with sess.as_default():
                ...
                feed_dict = {
                        cnn.input_x: x_submit,
                        cnn.dropout_keep_prob: 1.0}
                predictions = sess.run(cnn.predictions, feed_dict)
                predictions = [-1 if p == 0 else 1 for p in predictions] 
                with open("../twitter-datasets/" + str(FLAGS.train_size) + ".csv", "w") as f:
                    f.write("Id,Prediction\n")
                    for i,p in enumerate(predictions):
                        f.write(str(id_submit[i]) + "," + str(p) + "\n")
