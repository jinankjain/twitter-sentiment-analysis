    ...
    def create_model(self, ckpt_file=None):
        if ckpt_file is None:
            ...
            if self.arch == "vanilla":
                ...
                temp_model.add(Dropout(2*DROPOUT))
                ...
                lstm_layer = LSTM(LSTM_SIZE, dropout=self.drop_prob,
                                  recurrent_dropout=self.drop_prob,
                                  implementation=2, unroll=True)
                temp_model.add(Bidirectional(lstm_layer, merge_mode='sum'))
            else:
            ...
            openai_model.add(Embedding(4096, self.embedding_layer.output_dim, input_length=4096))
            ...
            result = Model(inputs=[first_input, second_input],outputs=[final])
            self.model=result
